  ...
  <div class="login-container">
    ...
    <form class="login-form" @submit.prevent="handleLogin">
      <div class="form-group">
        <label for="loginIdentifier">账号/ID</label>
        <input 
          type="text" 
          id="loginIdentifier" 
...
const handleLogin = async () => {
  ...
  try {
    const result = await userStore.login(loginIdentifier.value, password.value)
    if (result.success) {
      // 登录成功，跳转到首页
      router.push('/')
    } else {
...
export const useUserStore = defineStore('user', {
  ...
  getters: {
    ...
    account: (state) => state.currentUser?.user_name || state.currentUser?.id || '',
  ...
  actions: {
    ...
    // 用户登录
    async login(loginIdentifier, password, rememberMe = false) {
      this.isLoading = true
      this.error = null
      
      try {
        const result = await normalUserAPI.login(loginIdentifier, password)
        ...
        if (result.success) {
          ...
          // 如果勾选了记住我，保存到本地存储
          if (rememberMe) {
            localStorage.setItem('rememberedUsername', loginIdentifier)
          ...
          this.error = result.error || '登录失败，请稍后重试'
          return {
            success: false,
            error: this.error
          }
    ...
    async updateAvatar(file) {
      try {
        ...
        // 调用 API 上传头像
        const result = await normalUserAPI.uploadAvatar(file, this.currentUser.id)
        
        if (result.success) {
          // 更新用户头像URL
          this.currentUser.user_image = result.data
          // 保存更新后的用户信息到本地存储
