...
<template>
  <div class="game-card-wrapper">
    ...
    >
      <div class="game-image vertical">
        <img :src="game.libraryImage || game.image" :alt="game.name" />
      </div>
      <div class="game-info vertical">
          <div class="game-price-container" :class="{'isdiscount': game.discount !== 1}">
            ...
            <!-- 折扣价格 -->
            <span class="game-price" :class="{'discount': game.discount !== undefined && game.discount < 1 && game.price > 0}">{{ calculateDiscountPrice(game.price, game.discount) === '免费' ? '' : '￥' }}{{ calculateDiscountPrice(game.price, game.discount) }}</span>
          </div>
        </div>
    </router-link>
  </div>
